...
class TwoThreeFourTree:
    ...
    def split(self):
        ...
        if self.parent: #Dit is het geval voor wanneer de ouder van de node bestaat
            ...
            if len(self.parent.items) == 1:
                if self.parent.items[0].key < self.items[1].key: #We kijken hier naar het geval waar de huidige node de rightsubtree is van zijn parent
                    self.parent.items.append(self.items[1]) #We voegen de 2de item van de huidige node toe aan de oudernode
                    self.parent.middlesubtree = TwoThreeFourTree()
                    self.parent.middlesubtree.items.append(self.items[0]) #We maken hier een nieuwe subtree aan voor de parent en voegen daar de eerste item van de huidige node aan toe
                    ...
                    self.parent.items.insert(0,self.items[1]) #We voegen het 2de item van de huidige node toe aan het begin van de parent node
                ...
                if self.parent.items[1].key < self.items[1].key: #We kijken hier naar het geval waar de huidige node de rightsubtree is van zijn parent
                    self.parent.items.append(self.items[1]) #We voegen hier het 2de item van de huidige node toe aan de node van de parent
                    ...
                    self.rmiddlesubtree = None
                    self.lmiddlesubtree = None
                elif self.parent.items[0].key < self.items[1].key < self.parent.items[1].key: #We kijken hier naar het geval waar de huidige node de middlesubtree is van zijn parent
                    self.parent.items.insert(1,self.items[1]) #We voegen in onze parent de 2de item van de huidige node toe
                    ...
                    self.parent.middlesubtree = None
                elif self.parent.items[0].key > self.items[1].key: #We kijken hier naar het geval waar de huidige node de leftsubtree is van zijn parent
                    ...
                    self.parent.items.insert(0,self.items[1]) #We voegen aan de parent het middelste item van de huidige node aan toe
    ...
    def inordersuccesor(self,item):
        ...
        if len(self.items) == 3:
            if self.items[0].key == item: #Als het 1ste key van de huidige node gelijk is aan item kijken we verder in de lmiddlesubtree
                if self.leftsubtree:
                    ...
                    return self.items[0]
            if self.items[1].key == item: #Als het 2de key van de huidige node gelijk is aan item kijken we verder in de rmiddlesubtree
                if self.leftsubtree:
                    return self.rmiddlesubtree.inordersuccesor(item)
                else: #Als dit echter een blad is returnen we het item met de key waaraan onze item gelijk is
                    return self.items[1]
            if self.items[2].key == item: #Als het 3de key van de huidige node gelijk is aan item kijken we verder in de rightsubtree
                if self.leftsubtree:
    ...
    def deleteItem(self,key):
        ...
        if not self.items: #We kijken eerst of de huidige node leeg is , zo ja return False
            return False
        if len(self.items) == 1 and self.parent != None: #Dit is het geval voor als we een 2 knoop hebben dat niet de root is
        ...
        if len(self.items) == 1 and self.items[0].key != key and len(self.leftsubtree.items) == 1 and len(self.rightsubtree.items) == 1 or len(self.items) == 1 and self.items[0].key == key and len(self.leftsubtree.items) == 1 and len(self.rightsubtree.items) == 1 and self.parent == None:
